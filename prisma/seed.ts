...
const DUMMY_PRODUCT_SLUGS = [
  ...
  "donic-balls-3-star",
  "table-tennis-net-system",
  "wooden-blade-classic",
  "black-long-sleeve-top",
  "table-tennis-rubber-sheet",
  "cleaner-spray-sponge",
  "speed-glue-alternative",
];
...
async function main() {
  ...
  const deleted = await prisma.product.deleteMany({
    where: { slug: { in: DUMMY_PRODUCT_SLUGS } },
  });
  if (deleted.count > 0) {
    console.log(`Removed ${deleted.count} dummy products from seed`);
  }
