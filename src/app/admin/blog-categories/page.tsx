...
export default function AdminBlogCategoriesPage() {
  ...
  const [modal, setModal] = useState<"new" | BlogCategory | null>(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (modal === "new") {
        ...
        const res = await fetch(`/api/admin/blog-categories/${modal.id}`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload),
        });
  ...
  async function handleDelete(c: BlogCategory) {
    if (!confirm(`Delete blog category "${c.name}"? Posts in this category must be moved or deleted first.`)) return;
    try {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="font-sans text-2xl font-semibold text-text">Blog categories</h1>
        ...
        <div className="border border-border rounded-lg bg-surface overflow-hidden">
          <table className="w-full font-sans text-sm">
            ...
            <tbody>
              {categories.length === 0 ? (
                <tr>
                  <td colSpan={3} className="p-6 text-muted text-center">No blog categories yet</td>
                </tr>
              ) : (
                categories.map((c) => (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/30" onClick={() => !saving && setModal(null)}>
          <div className="bg-bg border border-border rounded-lg shadow-lg w-full max-w-md p-6" onClick={(e) => e.stopPropagation()}>
            <h2 className="font-sans text-lg font-semibold text-text mb-4">
              {modal === "new" ? "New blog category" : "Edit blog category"}
            </h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <p className="mt-1 font-sans text-xs text-muted">
                  URL-friendly (e.g., tips-tricks, equipment-reviews)
                </p>
              </div>
